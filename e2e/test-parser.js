...
const FIXTURE_PATH = path.join(__dirname, '../src/__tests__/fixtures/1759320900.dat');
...
function parsePost(postStr, resNum) {
    ...
    try {
        ...
        if (dateAndIdIdx < 2 || dateAndIdIdx + 1 >= splitParts.length) {
            console.log(`Line ${resNum}: Invalid structure - dateAndIdIdx=${dateAndIdIdx}, parts=${splitParts.length}`);
            return null;
        }
        ...
        const authorId = headerSplit.length > 1 ? headerSplit.slice(1).join("ID:").trim() : "";

        console.log(`Line ${resNum}: OK - author="${authorName}", date="${rawDateStr}", id="${authorId}", content="${content.substring(0, 50)}..."`);
...
try {
    ...
    console.log(`Success rate: ${((successCount / testLines.length) * 100).toFixed(1)}%`);
