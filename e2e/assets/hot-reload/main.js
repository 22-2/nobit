...
function around1(obj, method, createWrapper) {
	const inherited = obj[method],
		...
		original = hadOwn
			? inherited
			: function () {
					return Object.getPrototypeOf(obj)[method].apply(
						this,
						arguments,
					);
				};
...
// hot-reload.ts
var watchNeeded =
	!import_obsidian.Platform.isMacOS && !import_obsidian.Platform.isWin;
var HotReload = class extends import_obsidian.Plugin {
	constructor() {
		...
		// path -> Stat
		this.run = taskQueue();
		this.reindexPlugins = (0, import_obsidian.debounce)(
	...
	async watch(path) {
		...
		if (
			!(adapter instanceof import_obsidian.FileSystemAdapter) ||
			adapter.watchers?.hasOwnProperty(path)
		)
			return;
	...
	checkVersions() {
		return Promise.all(
			Object.values(this.pluginNames).map(this.checkVersion),
		);
	}
	...
	requestReload(plugin) {
		...
		const reloader =
			...
			(this.pluginReloaders[plugin] = (0, import_obsidian.debounce)(
				() => this.run(() => this.reload(plugin).catch(console.error)),
				750,
				true,
			));
		reloader();
...
var SettingReloader = class extends import_obsidian.Component {
	...
	onload() {
		...
		this.register(
			around(import_obsidian.Plugin.prototype, {
				addSettingTab(next) {
