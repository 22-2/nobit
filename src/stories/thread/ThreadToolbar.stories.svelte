	...
	<CenterDecorator padding="var(--size-4-4)" minHeight="var(--size-4-25)">
		<div style="width: 100%; max-width: 600px;">
			...
			<div style="margin-bottom: var(--size-4-4);">
				<h4
					style="color: var(--text-normal); margin-bottom: var(--size-4-2); font-size: 0.9em;"
				>
				...
				<ThreadToolbar
					...
					onRefresh={() => {
						...
						console.log(
							"Refresh clicked - ThreadManager.refreshThread() called",
						);
					}}
					onWriteButtonClick={fn()}
				/>
			...
			<div style="margin-bottom: var(--size-4-4);">
				...
				<ThreadToolbar
					isCoolingDown={false}
					isLoading={createMockThreadManager({ isLoading: true })
						.isLoading}
					onRefresh={() => {
			...
			<div style="margin-bottom: var(--size-4-4);">
				...
				<ThreadToolbar
					...
					onRefresh={() => {
						...
						mockManager.refreshThread();
						console.log(
							"Refresh clicked - Reloading current thread",
						);
					}}
					onWriteButtonClick={fn()}
				/>
			...
			>
				...
				>
					注意:
					この統合テストでは、ThreadManagerのモックを使用してツールバーの動作を検証します。
...
>
	<CenterDecorator padding="var(--size-4-4)" minHeight="var(--size-4-20)">
		<div style="width: 100%; max-width: 600px;">
			...
			<div
				style="padding: var(--size-4-3); background-color: var(--background-modifier-error); border-radius: var(--radius-s);"
			>
				<p
					style="color: var(--text-error); font-size: 0.85em; margin: 0;"
				>
...
>
	<div
		style="display: flex; flex-direction: column; gap: var(--size-4-4); padding: var(--size-4-4);"
	>
		<div>
			<h4
			...
			<div
				style="width: 600px; border: 1px solid var(--background-modifier-border);"
			>
				<ThreadToolbar
					isCoolingDown={false}
					isLoading={false}
		...
		<div>
			...
			<div
				style="width: 400px; border: 1px solid var(--background-modifier-border);"
			>
				<ThreadToolbar
					isCoolingDown={false}
					isLoading={false}
		...
		<div>
			...
			<div
				style="width: 300px; border: 1px solid var(--background-modifier-border);"
			>
				<ThreadToolbar
					isCoolingDown={false}
					isLoading={false}
